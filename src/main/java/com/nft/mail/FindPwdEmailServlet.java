...
public class FindPwdEmailServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int result;
		String e_wallet2 = request.getParameter("e_wallet2");				
		String userid = request.getParameter("userid");
		String phone2 = request.getParameter("phone2");
		int emailAgree2 = Integer.parseInt(request.getParameter("emailAgree2"));
		request.setAttribute("e_wallet2", e_wallet2);
		request.setAttribute("userid", userid);
		request.setAttribute("phone2", phone2);
		request.setAttribute("emailAgree2", emailAgree2);
		
		EmailDao eDao = EmailDao.getInstance();
		result = eDao.checkFindPwdEmail(userid, phone2, e_wallet2);
		System.out.println(result);
		if(result == 0) {
			request.setAttribute("conMessage", 2);
			RequestDispatcher dispatcher = request.getRequestDispatcher("member/findIdPwd.jsp");
			dispatcher.forward(request, response);
		}else {
			request.setAttribute("conMessage", 3);
		}
