...
public class Main {
	...
	public String makeNFT(String nftUrl, String fileName, String select, String uploadFilePath) {
		...
		try {
			...
			image1 = ImageIO.read(new File(uploadFilePath+"\\"+nftUrl));
			...
			if(select.equals("1")) {
				...
				// 서버에 이미지 두어야 함...!
				image2 = ImageIO.read(new File(uploadFilePath+"\\"+"할로윈1.png"));
				//image2 = ImageIO.read(getClass().getClassLoader().getResource("할로윈1.png"));
				//image2 = ImageIO.read(new File(path+"/WebContent/upload/할로윈1.png"));
				System.out.println();
			} if(select.equals("2")) {
			...
			g.drawImage(image2, 0, 0, null);
					...
					try {
						...
						ImageIO.write(combinedImage, "PNG", new File(uploadFilePath+"\\"+fileName));
					} catch(IOException e) {
